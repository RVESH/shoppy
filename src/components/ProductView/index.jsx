...
export default function ProductView({ product, onClose, onWishlist, isWishlisted = false }) {
  ...
  const [showOrder,   setShowOrder]   = useState(false); // ← OrderModal state

  const handleKey = useCallback((e) => {
    if (e.key === "Escape" && !showOrder) onClose();
  }, [onClose, showOrder]);
  ...
  // ── "Order on WhatsApp" click ──────────────────────────
  // Product cart mein add karo + OrderModal kholo
  function handleWaOrderClick() {
    // Pehle product cart mein daalo (agar nahi hai toh)
    if (!inCart) {
      addToCart(product, quantity, variants);
    }
    // OrderModal kholo
  ...
  function handleOrderClose() {
    setShowOrder(false);
    onClose(); // ProductView bhi band karo
  }

  // ── Agar OrderModal open hai toh wahi dikhao ──────────────
  if (showOrder) {
    return <OrderModal onClose={handleOrderClose} />;
  }
        ...
        <div className="pvBody">
          ...
          <div className="pvLeft">
            <div className="pvImageWrap">
              ...
              <button
                className={`pvHeart ${isWishlisted ? "pvHeartActive" : ""}`}
            ...
            <div className="pvDeliveryRow">
              {product.deliveryAvailable && (
                <span className="pvDeliveryChip">🚚 Free Delivery</span>
              )}
              {!product.isService && product.stock > 20 && (
                <span className="pvStockChip">✓ In Stock</span>
              )}
          ...
          <div className="pvRight">
            ...
            <div className="pvPricing">
              ...
              {product.isService && product.priceType === "variable" && (
                <span className="pvOnwards">onwards</span>
              )}
              {product.mrp > product.price && (
              ...
              <div key={key} className="pvVariantGroup">
                <div className="pvVariantLabel">
                  ...
                  {variants[key]
                    ...
                    : <span className="pvVariantHint">Select</span>
                  }
                </div>
            ...
            {/* Quantity (only if not in cart and not a service) */}
            ...
            {product.isService && product.documentsRequired?.length > 0 && (
              <div className="pvDocsBox">
                ...
                <div className="pvDocsList">
                  {product.documentsRequired.map(d => (
                    <span key={d} className="pvDocChip">{d}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="pvCta">
              ...
              {!product.isService ? (
                !inCart ? (
                  <button
                    className={`pvBookBtn ${justAdded ? "pvBookBtnDone" : ""}`}
                  ...
                  <div className="pvInCartRow">
                    <span className="pvInCartLabel">✓ In Cart</span>
                    <div className="pvQtyControl">
                ...
                <button
                  className="pvBookBtn pvBookBtnService"
                  onClick={handleAddToCart}
                >
                  📋 Add Enquiry to Cart
                </button>
              ...
              {/* ── WhatsApp Order Button → Opens OrderModal ── */}
              <button className="pvWaBtn" onClick={handleWaOrderClick}>
                <WaSvg size={18} />
                Order on WhatsApp
              </button>
            ...
            <div className="pvTabs">
              <div className="pvTabNav">
                {[
                  ...
                  <button
                    key={k}
                    className={`pvTabBtn ${tab === k ? "pvTabOn" : ""}`}
                    onClick={() => setTab(k)}
                  >
                    {l}
                  </button>
              ...
              <div className="pvTabBody">
                {tab === "desc" && (
                  <p className="pvTabText">{product.description || "No description available."}</p>
                )}
                {tab === "specs" && (
                  ...
                  <div className="pvTabVarList">
                    {variantEntries.map(([key, vals]) => (
                      <div key={key} className="pvTabVarRow">
                        <strong>{key}:</strong>
                        <div>
                          {vals.map(v => <span key={v} className="pvTabVarChip">{v}</span>)}
                        </div>
                      </div>
                    ))}
                  </div>
