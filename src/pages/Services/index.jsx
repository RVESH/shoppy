...
const ServiceCard = ({ service, onOpen }) => {
  ...
  return (
    <div className={`serviceCard ${expanded ? "serviceCardExpanded" : ""}`}>
      <div className="scTop">
        <div className="scIconBox" onClick={() => onOpen(service)} style={{cursor:"pointer"}}>
          {subCatIcons[service.subCategory] || "🛠️"}
        </div>
        <div className="scMeta">
          <div className="scSubCat">{service.subCategory}</div>
          <h3 className="scName" onClick={() => onOpen(service)} style={{cursor:"pointer"}}>{service.name}</h3>
      ...
      <div className="scPriceRow">
        <div className="scPrice">₹{service.price}{service.priceType === "variable" && <span className="scOnwards">+</span>}</div>
        <div className="scPriceType">{service.priceType === "variable" ? "Variable Price" : "Fixed Price"}</div>
        {service.deliveryAvailable && <div className="scDelivery">🚚 Online Delivery</div>}
      </div>
      <p className="scDesc">{service.description}</p>
      <div className="scTime">🕐 <strong>Processing:</strong> {service.processingTime}</div>
          ...
          {expanded && (
            <div className="scDocsList">
              {service.documentsRequired.map((doc, i) => (
                <div key={i} className="scDocItem"><span className="scDocDot">✓</span>{doc}</div>
              ))}
            </div>
          )}
        </>
      ...
      <div className="scActions">
        <button className="scViewBtn" onClick={() => onOpen(service)}>🔍 View Details</button>
        <a href="https://wa.me/916206869543" target="_blank" rel="noreferrer" className="scWaBtn">
...
export default function Services() {
  ...
  const filtered = activeSubCat === "All" ? services : services.filter(s => s.subCategory === activeSubCat);
    ...
    <div className="servicesPage">
      ...
      <div className="srvPageHeader">
        <div className="srvPageHeaderInner">
          ...
          <p className="srvPageSubtitle">From PAN card to resume — everything in one place, instant service!</p>
      ...
      <div className="howItWorks">
        <div className="howItWorksInner">
          <h2 className="howTitle">How does it work? 🤔</h2>
          <div className="howSteps">
            {[
              { icon:"📱", step:"1", title:"Message on WhatsApp", desc:"Tell us what you need" },
              { icon:"📋", step:"2", title:"Bring Documents",      desc:"Keep required documents ready" },
              { icon:"⚡", step:"3", title:"Work Gets Done",       desc:"Same-day processing" },
              { icon:"✅", step:"4", title:"Done!",                desc:"Collect via delivery or walk-in" },
            ].map(s => (
      ...
      <div className="srvFilterBar">
        <div className="srvFilterBarInner">
          <button className={`srvFilterBtn ${activeSubCat === "All" ? "srvFilterBtnActive" : ""}`} onClick={() => setActiveSubCat("All")}>
          ...
          {subCategories.map(sub => (
            <button key={sub}
              ...
              <span className="srvFilterCount">{services.filter(s => s.subCategory === sub).length}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="srvGrid">
        <div className="srvGridInner">
          <div className="srvResultInfo"><strong>{filtered.length}</strong> services available</div>
          {loading ? (
            <div style={{textAlign:"center",padding:"60px",color:"#78716c"}}>⏳ Services are loading...</div>
          ) : (
            <div className="servicesGrid">
              {filtered.map(s => <ServiceCard key={s.id} service={s} onOpen={openProduct} />)}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="srvCta">
        <div className="srvCtaInner">
          ...
          <h2 className="srvCtaTitle">Need a custom service?</h2>
          <p className="srvCtaSubtitle">Not in the list? No problem — ask on WhatsApp!</p>
          <a href="https://wa.me/916206869543" target="_blank" rel="noreferrer" className="srvCtaWaBtn">
            <WaSvg size={20} /> Ask on WhatsApp
          </a>
        </div>
      </div>
    </div>
