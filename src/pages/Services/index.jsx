...
// ── ServiceCard — poori card clickable, sirf WA button alag ──
const ServiceCard = ({ service, onOpen, onBook }) => {
  ...
  return (
    <div className="serviceCard" onClick={() => onOpen(service)}>
      ...
      {service.documentsRequired?.length > 0 && (
        <>
          <button className="scDocsToggle" onClick={handleDocsToggle}>
            📋 Documents Required
      ...
      <div className="scTapHint">👆 Card tap karke details dekho</div>

      <button className="scWaBtn" onClick={handleBook}>
        ...
        <div className="srvGridInner">
          ...
          {loading ? (
            ...
            <div className="servicesGrid">
              {filtered.map(s => (
                <ServiceCard key={s.id} service={s} onOpen={openProduct} onBook={handleBook} />
              ))}
            </div>
          )}
        </div>
